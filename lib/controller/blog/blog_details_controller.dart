...
class BlogDetailsController extends StateNotifier<BaseState> {
  ...
  Future addRemoveFavoriteBlog({blogId, isFavorite}) async {
    state = const LoadingState();
    ...
    try {
      responseBody = await Network.handleResponse(
        await Network.postRequest(API.addRemoveFavoriteBlog, requestBody),
      );
      if (responseBody != null) {
        blogModel = BlogModel.fromJson(responseBody['student'][0]);
      ...
      print('addRemoveFavoriteBlog() error = $error');
      print(stackTrace);
      state = const ErrorState();
    }
  }
